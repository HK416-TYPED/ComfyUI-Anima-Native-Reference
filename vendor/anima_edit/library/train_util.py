...
import torch
from _anima_native_ref_vendor.library.device_utils import init_ipex, clean_memory_on_device
from _anima_native_ref_vendor.library.strategy_base import LatentsCachingStrategy, TokenizeStrategy, TextEncoderOutputsCachingStrategy, TextEncodingStrategy
...
from _anima_native_ref_vendor.library import custom_train_functions, sd3_utils
...
from _anima_native_ref_vendor.library.lpw_stable_diffusion import StableDiffusionLongPromptWeightingPipeline
from _anima_native_ref_vendor.library.sdxl_lpw_stable_diffusion import SdxlStableDiffusionLongPromptWeightingPipeline
import _anima_native_ref_vendor.library.model_util as model_util
import _anima_native_ref_vendor.library.huggingface_util as huggingface_util
import _anima_native_ref_vendor.library.sai_model_spec as sai_model_spec
import _anima_native_ref_vendor.library.deepspeed_utils as deepspeed_utils
from _anima_native_ref_vendor.library.utils import setup_logging, resize_image, validate_interpolation_fn
...
try:
    ...
    from _anima_native_ref_vendor.library.jpeg_xl_util import get_jxl_size
...
class ImageInfo:
    def __init__(
        ...
        self.reference_slot_ids: Optional[List[int]] = (
            [int(slot_id) for slot_id in reference_slot_ids] if reference_slot_ids is not None else None
        )
        if self.reference_slot_ids is not None and len(self.reference_slot_ids) != len(self.reference_image_paths):
            raise ValueError(
                f"reference_slot_ids length ({len(self.reference_slot_ids)}) must match "
                f"reference_image_paths length ({len(self.reference_image_paths)}) for {absolute_path}."
            )
        ...
        # Ordered native-reference latents are deliberately host-RAM only. The
        # companion path snapshot makes later path reordering fail closed.
        self.reference_latents: Optional[List[torch.Tensor]] = None
        self.reference_latent_paths: Optional[List[str]] = None
    ...
    def get_cached_reference_latents(
        self,
        selected_paths: Optional[List[str]] = None,
        *,
        require_complete: bool = False,
    ) -> Optional[List[torch.Tensor]]:
        """Return a validated ordered reference cache, or ``None`` if absent."""

        if self.reference_latents is None:
            if require_complete:
                raise ValueError(f"Reference latent cache is missing for {self.absolute_path}")
            return None

        expected_paths = list(self.reference_image_paths)
        selected_paths = expected_paths if selected_paths is None else list(selected_paths)
        if not expected_paths or selected_paths != expected_paths:
            raise ValueError(
                f"Cached reference paths cannot be remapped for {self.absolute_path}: "
                f"expected {expected_paths}, selected {selected_paths}."
            )
        if self.reference_latent_paths is None or list(self.reference_latent_paths) != expected_paths:
            raise ValueError(f"Reference latent cache path snapshot is stale for {self.absolute_path}")
        if len(self.reference_latents) != len(expected_paths):
            raise ValueError(
                f"Reference latent cache count ({len(self.reference_latents)}) does not match path count "
                f"({len(expected_paths)}) for {self.absolute_path}."
            )
        for index, latent in enumerate(self.reference_latents):
            if not torch.is_tensor(latent) or latent.ndim != 3:
                raise ValueError(
                    f"Reference latent {index} for {self.absolute_path} must be a [C,H,W] tensor."
                )
            if latent.device.type != "cpu" or not latent.is_contiguous():
                raise ValueError(
                    f"Reference latent {index} for {self.absolute_path} must be contiguous host memory."
                )
            if not torch.isfinite(latent).all():
                raise ValueError(f"Reference latent {index} for {self.absolute_path} is non-finite.")
        return list(self.reference_latents)
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    @staticmethod
    def select_reference_slot_ids(image_info: ImageInfo, selected_paths: List[str]) -> List[int]:
        """Preserve explicit logical slots; only default to list order when absent."""

        explicit = getattr(image_info, "reference_slot_ids", None)
        if explicit is not None and image_info.reference_image_paths:
            if list(selected_paths) != list(image_info.reference_image_paths):
                raise ValueError("Explicit reference_slot_ids cannot be remapped to different reference paths.")
            return list(explicit)
        return list(range(len(selected_paths)))
    ...
    def __getitem__(self, index, _retry_count: int = 0):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            # Once a complete ordered reference cache exists there is no
            # per-step reference file I/O. A partial/stale cache raises here
            # instead of silently falling back to a different conditioning path.
            cached_reference_latents = image_info.get_cached_reference_latents()
            if getattr(subset, "spawner_jsonl_skip_missing", False) and cached_reference_latents is None:
                try:
        ...
        selected_reference_image_paths = [
            self.select_reference_image_paths(self.image_data[image_key]) for image_key in included_image_keys
        ]
        example["reference_image_paths"] = selected_reference_image_paths
        example["reference_slot_ids"] = [
        ...
        if all(latents is None for latents in cached_reference_latents):
            example["reference_latents"] = None
        elif any(latents is None for latents in cached_reference_latents):
            raise ValueError("A batch cannot mix cached and uncached ordered reference latents.")
        else:
            # Match the existing online loader contract: each physical ref is
            # represented as [1,C,H,W] inside the per-sample list.
            example["reference_latents"] = [
                [latent.unsqueeze(0) for latent in sample_latents]
                for sample_latents in cached_reference_latents
            ]
    ...
    def random_mask(im_shape):
        from _anima_native_ref_vendor.library.mask_generator import random_mask as _random_mask
        ...
        for subset in subsets:
            ...
            if os.path.exists(subset.metadata_file):
                if getattr(subset, "spawner_jsonl_pairs", False):
                    ...
                    logger.info(f"loading existing JSONL metadata: {subset.metadata_file}")
                    # Backward compatible with the historical image_path/caption rows,
                    # while directly accepting anima-ordered-reference-edit/v1.
                    metadata = {}
                    ...
                    with open(subset.metadata_file, "rt", encoding="utf-8") as f:
                        for line_no, line in enumerate(f, start=1):
                            ...
                            line_md = json.loads(line)
                            is_ordered_edit = (
                                line_md.get("schema") == "anima-ordered-reference-edit/v1"
                                or line_md.get("schema_version") == "anima-ordered-reference-edit/v1"
                                or "target_path" in line_md
                            )
                            if is_ordered_edit:
                                target_path = line_md.get("target_path")
                                references = line_md.get("ordered_references")
                                slot_ids = line_md.get("reference_slot_ids")
                                instruction = line_md.get("instruction")
                                if not isinstance(target_path, str) or not target_path:
                                    raise ValueError(f"ordered edit JSONL line {line_no} has no target_path")
                                if not isinstance(instruction, str):
                                    raise ValueError(f"ordered edit JSONL line {line_no} has no instruction")
                                if not isinstance(references, list) or not all(
                                    isinstance(path, str) and path for path in references
                                ):
                                    raise ValueError(f"ordered edit JSONL line {line_no} has invalid ordered_references")
                                if slot_ids is not None and (
                                    not isinstance(slot_ids, list) or len(slot_ids) != len(references)
                                ):
                                    raise ValueError(
                                        f"ordered edit JSONL line {line_no} reference_slot_ids must match ordered_references"
                                    )
                                target_path = (
                                    os.path.normpath(target_path)
                                    if os.path.isabs(target_path)
                                    else os.path.normpath(os.path.join(metadata_dir, target_path))
                                )
                                reference_paths = [
                                    os.path.normpath(path)
                                    if os.path.isabs(path)
                                    else os.path.normpath(os.path.join(metadata_dir, path))
                                    for path in references
                                ]
                                if os.path.normcase(os.path.abspath(target_path)) in {
                                    os.path.normcase(os.path.abspath(path)) for path in reference_paths
                                }:
                                    raise ValueError(f"ordered edit JSONL line {line_no} leaks target into references")
                                image_md = {
                                ...
                                image_key = target_path
                                if image_key in metadata:
                                    sample_id = line_md.get("sample_id", line_no)
                                    image_key = f"{target_path}#ordered_edit_{sample_id}"
                                metadata[image_key] = image_md
                            else:
                                image_path = line_md.get("image_path")
                                if not isinstance(image_path, str) or not image_path:
                                    raise ValueError(f"JSONL line {line_no} has no image_path")
                                image_md = {"caption": line_md.get("caption", "")}
                                if "image_size" in line_md:
                                    image_md["image_size"] = line_md["image_size"]
                                if "tags" in line_md:
                                    image_md["tags"] = line_md["tags"]
                                metadata[image_path] = image_md
                else:
            ...
            for image_key in image_keys_sorted_by_length_desc:
                ...
                reference_slot_ids = img_md.get("reference_slot_ids")
                if reference_slot_ids is not None:
                    if not isinstance(reference_slot_ids, (list, tuple)):
                        raise ValueError(f"reference_slot_ids must be a list for metadata item {image_key}")
                    reference_slot_ids = [int(slot_id) for slot_id in reference_slot_ids]
                    if any(slot_id < 0 for slot_id in reference_slot_ids):
                        raise ValueError(f"reference_slot_ids must be non-negative for metadata item {image_key}")
                    if len(set(reference_slot_ids)) != len(reference_slot_ids):
                        raise ValueError(f"reference_slot_ids must be unique for metadata item {image_key}")
                    if len(reference_slot_ids) != len(reference_image_paths):
                        raise ValueError(
                            f"reference_slot_ids length must match reference_image_paths for metadata item {image_key}"
                        )
...
def load_prompts(prompt_file: str) -> List[Dict]:
    ...
    for i in range(len(prompts)):
        ...
        if isinstance(prompt_dict, str):
            from _anima_native_ref_vendor.library.train_util import line_to_prompt_dict
...
def sample_image_inference(
    ...
    if getattr(args, "train_inpainting", False):
        ...
        if image_path:
            from _anima_native_ref_vendor.library.mask_generator import wobbly_ellipse_mask as _gen_mask
